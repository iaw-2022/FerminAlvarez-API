...
const getBooksByBookshopId = async(req, res) => {
    if(!isNaN(req.params.Id)){
        const responseBooks = 
        await database.query('SELECT books."ISBN", books.name, publisher, total_pages, published_at, image_link, categories.name as category FROM (books JOIN categories ON books.category = categories.id JOIN has ON books."ISBN" = has."ISBN") WHERE has."Bookshop" = $1', [req.params.Id]);
